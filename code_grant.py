...
class OAuth2ClientInfo:
    ...
    @staticmethod
    def from_oidc_endpoint(oidc_config_endpoint: str, client_id: str, scopes: list[str], client_secret: str | None = None):
        """
        Constructs an OAuth2ClientInfo object from an OpenID Connect discovery endpoint.

        Args:
            oidc_config_endpoint: URL of the OpenID Connect discovery endpoint.
            client_id: The client ID for authentication.
            scopes: A list of scopes to request.
            client_secret: The client secret for authentication (optional).

        Returns:
            An OAuth2ClientInfo object.

        Raises:
            ValueError: If the OpenID Connect discovery endpoint does not provide the required information.
        """
        config = load_oidc_config(oidc_config_endpoint)

        if "authorization_endpoint" not in config or "token_endpoint" not in config:
            raise ValueError("Invalid OpenID Connect discovery document")

        return OAuth2ClientInfo(
            authorization_url=config["authorization_endpoint"],
            token_url=config["token_endpoint"],
            client_id=client_id,
            client_secret=client_secret,  # Pass client_secret if provided
            scopes=scopes,
        )
